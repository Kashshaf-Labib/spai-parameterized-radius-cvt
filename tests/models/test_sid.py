...
class TestPatchBasedMFViTLearnableRadius(unittest.TestCase):

    @staticmethod
    def _build_model(learnable_radius: bool) -> sid.PatchBasedMFViT:
        features_num: int = 2
        vit = vision_transformer.VisionTransformer(
            img_size=224,
            patch_size=16,
    ...
    def test_gradient_flows_to_radius(self) -> None:
        model = self._build_model(learnable_radius=True)
        model.train()

        # The backbone should be frozen through requires_grad, instead of no_grad.
        self.assertTrue(all(not p.requires_grad for p in model.mfvit.vit.parameters()))
        self.assertAlmostEqual(model.get_masking_radius(), 16.0)

        x: torch.Tensor = torch.rand((2, 3, 224, 224))
        y: torch.Tensor = torch.tensor([.0, 1.])
        out: torch.Tensor = model(x).squeeze(dim=-1)
        loss: torch.Tensor = nn.functional.binary_cross_entropy_with_logits(out, y)
        loss.backward()

        radius: nn.Parameter = model.mfvit.soft_frequency_mask.radius
        self.assertIsNotNone(radius.grad)
        self.assertTrue(torch.isfinite(radius.grad).all())
        self.assertGreater(radius.grad.abs().item(), .0)

        # The frozen backbone should not receive any gradients.
        self.assertTrue(all(p.grad is None for p in model.mfvit.vit.parameters()))

        # The trainable heads should receive gradients.
        self.assertTrue(all(p.grad is not None for p in model.cls_head.parameters()))

    def test_fixed_checkpoint_loads_into_learnable_model(self) -> None:
        fixed_model = self._build_model(learnable_radius=False)
        learnable_model = self._build_model(learnable_radius=True)

        msg = learnable_model.load_state_dict(fixed_model.state_dict(), strict=False)
        self.assertEqual(msg.missing_keys, ["mfvit.soft_frequency_mask.radius"])
        self.assertEqual(msg.unexpected_keys, ["mfvit.frequencies_mask"])
        self.assertAlmostEqual(learnable_model.get_masking_radius(), 16.0)
