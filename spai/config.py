...
_C.MODEL.RESUME = ''
# Path to a full model checkpoint (e.g. the released spai.pth) used to initialize the
# whole model for fine-tuning. Loaded with strict=False, so components that do not
# exist in the checkpoint (e.g. the learnable masking radius) keep their initialization.
...
# When set to True, the frequency masking radius becomes a learnable parameter,
# initialized to MODEL.FRE.MASKING_RADIUS, instead of a fixed binary circular mask.
_C.MODEL.FRE.LEARNABLE_MASKING_RADIUS = False
# Width, in frequency bins, of the soft transition zone of the learnable mask. Only
# used when MODEL.FRE.LEARNABLE_MASKING_RADIUS is set to True.
_C.MODEL.FRE.MASK_TEMPERATURE = 1.0
...
# Learning rate for the learnable masking radius parameter. As the radius is measured
# in frequency bins, it requires a much higher learning rate than the rest of the
# network to meaningfully move during training. Only used when
# MODEL.FRE.LEARNABLE_MASKING_RADIUS is set to True.
_C.TRAIN.RADIUS_LR = 0.01
...
def update_config(config, args):
    ...
    if _check_args('finetune_from'):
        config.MODEL.FINETUNE_FROM = args["finetune_from"]
