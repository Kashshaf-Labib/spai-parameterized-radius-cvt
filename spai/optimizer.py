...
def build_finetune_optimizer(config, model, logger):
    ...
    parameters = get_finetune_param_groups(
        ...
        radius_lr=config.TRAIN.RADIUS_LR)
...
def get_finetune_param_groups(model, logger, lr, weight_decay, get_layer_func, scales, skip_list=(), skip_keywords=(), radius_lr=None):
    ...
    for name, param in model.named_parameters():
        if not param.requires_grad:
            continue
        if name.endswith("soft_frequency_mask.radius") and radius_lr is not None:
            # The learnable masking radius is measured in frequency bins, a much larger
            # scale than the rest of the parameters, so it receives its own group with
            # a dedicated learning rate and no weight decay.
            group_name = "masking_radius"
