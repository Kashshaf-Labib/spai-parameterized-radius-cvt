...
class _NeptuneStub:
    """No-op replacement for a Neptune run, used when Neptune is unavailable.

    Supports the subset of the Neptune API used throughout this module, i.e.
    item assignment, `run["metric"].append(...)`, `run.sync()` and `run.stop()`.
    """
...
def init_neptune_run(name: str, tags: list[str]):
    """Initializes a Neptune run, falling back to a no-op stub when unavailable.

    Neptune is skipped when the DISABLE_NEPTUNE environment variable is set to a
    non-empty value, or when the initialization fails (e.g. no API token configured),
    so that training can run on environments like Kaggle without a Neptune account.
    TensorBoard logging remains always active, independently of Neptune.
    """
    if neptune is None or os.environ.get("DISABLE_NEPTUNE", ""):
        return _NeptuneStub()
    try:
        return neptune.init_run(name=name, tags=tags)
    except Exception as e:
        print(f"Neptune initialization failed ({e}). Continuing without Neptune logging.")
        return _NeptuneStub()
...
@click.option("--finetune-from", "finetune_from",
              type=click.Path(exists=True, dir_okay=False, path_type=Path),
              help="Path to a full model checkpoint (e.g. the released spai.pth) used to "
...
def train(
    ...
    if config.MODEL.FRE.LEARNABLE_MASKING_RADIUS:
        logger.info(f"Learnable masking radius enabled | "
                    f"initial r = {model_without_ddp.get_masking_radius():.4f} | "
                    f"temperature = {config.MODEL.FRE.MASK_TEMPERATURE} | "
                    f"radius lr = {config.TRAIN.RADIUS_LR}")
...
def train_model(
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        if config.MODEL.FRE.LEARNABLE_MASKING_RADIUS:
            masking_radius: float = model_without_ddp.get_masking_radius()
            logger.info(f"Masking radius | Epoch {epoch} | r = {masking_radius:.4f}")
            log_writer.add_scalar("train/masking_radius", masking_radius, epoch)
            neptune_run["train/masking_radius"].append(masking_radius)
...
def train_one_epoch(
    ...
    for idx, batch in enumerate(data_loader):
        ...
        if idx % config.PRINT_FREQ == 0:
            ...
            radius_info: str = ""
            if config.MODEL.FRE.LEARNABLE_MASKING_RADIUS:
                radius_info = f'masking_radius {model.get_masking_radius():.4f}\t'
            logger.info(
