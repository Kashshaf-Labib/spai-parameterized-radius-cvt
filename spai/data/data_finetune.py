...
class CSVDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        # The CSV file is read through a reader rooted at "/", so its path must be
        # absolute. A relative path (e.g. passed via --data-path) is resolved against
        # the current working directory, which is the intuitive behavior.
        csv_abs_path: pathlib.Path = pathlib.Path(self.csv_path)
        if not csv_abs_path.is_absolute():
            csv_abs_path = pathlib.Path.cwd() / csv_abs_path
        reader = readers.FileSystemReader(pathlib.Path("/"))
        self.entries: list[dict[str, Any]] = reader.read_csv_file(str(csv_abs_path))
