...
def load_finetune_checkpoint(
    ...
    """Initializes the whole model from a full checkpoint (e.g. the released spai.pth).

    The checkpoint is loaded with strict=False, so components that do not exist in the
    checkpoint (e.g. the learnable masking radius) keep their initialization, while
    obsolete checkpoint entries (e.g. the fixed frequencies mask, when a learnable
    radius is used) are ignored.
    """
    checkpoint_path = pathlib.Path(config.MODEL.FINETUNE_FROM)
    logger.info(f">>>>>>>>>> Fine-tuning from full checkpoint {checkpoint_path} ..........")
    checkpoint = torch.load(checkpoint_path, map_location="cpu", weights_only=False)
    checkpoint_model = checkpoint["model"] if "model" in checkpoint else checkpoint

    msg = model.load_state_dict(checkpoint_model, strict=False)
    if len(msg.missing_keys) > 0:
        logger.warning(f"Keys missing from the checkpoint (keeping their initialization): "
                       f"{msg.missing_keys}")
    if len(msg.unexpected_keys) > 0:
        logger.warning(f"Checkpoint keys not used by the model: {msg.unexpected_keys}")
