...
class PatchBasedMFViT(nn.Module):
    ...
    def get_masking_radius(self) -> float:
        """Returns the current value of the masking radius in frequency bins."""
        return self.mfvit.get_masking_radius()
...
class MFViT(nn.Module):
    ...
    def __init__(
        ...
        self.learnable_radius: bool = learnable_radius
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        low_freq, hi_freq = filters.filter_image_frequencies(
            x.float(), self.get_frequencies_mask()
        )
        ...
        if self.frozen_backbone:
            if self.learnable_radius and torch.is_grad_enabled():
                # The backbone weights are frozen through requires_grad=False, so the
                # graph of the filtered streams can be kept for the gradients to reach
                # the learnable masking radius. The original image stream does not
                # depend on the mask and is computed without a graph to save memory.
                with torch.no_grad():
                    x = self.vit(x)
    ...
    def get_frequencies_mask(self) -> torch.Tensor:
        """Returns the frequency mask, computing it from the radius when learnable."""
        if self.learnable_radius:
            return self.soft_frequency_mask()
        return self.frequencies_mask

    def get_masking_radius(self) -> float:
        """Returns the current value of the masking radius in frequency bins."""
        if self.learnable_radius:
            return self.soft_frequency_mask.get_radius()
        raise RuntimeError("Masking radius value tracking requires a learnable radius.")
    ...
    def export_onnx(self, export_file: pathlib.Path) -> None:
        ...
        class ExportableMFVit(nn.Module):
            ...
            def forward(self, x: torch.Tensor) -> torch.Tensor:
                ...
                low_freq, hi_freq = filters.filter_image_frequencies(
                    x.float(), outer_instance.get_frequencies_mask()
                )
