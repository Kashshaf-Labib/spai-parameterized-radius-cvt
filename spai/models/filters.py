...
class SoftCircularMask(nn.Module):
    """Circular low-pass frequency mask with a learnable radius.

    Differentiable replacement for `generate_circular_mask`. The hard `distance < radius`
    threshold is relaxed into a sigmoid boundary, so gradients can flow into the radius
    parameter and the masking radius can be learned end-to-end. Values close to 1 are
    produced for frequencies inside the radius and values close to 0 outside of it,
    matching the semantics of `generate_circular_mask`.

    :param input_size: Size of the (square) input image in pixels.
    :param initial_radius: Initial value of the masking radius, in frequency bins from
        the center of the shifted spectrum.
    :param temperature: Width, in frequency bins, of the soft transition zone between
        the passed and the masked frequency bands. Lower values approximate the binary
        mask more closely, at the cost of sharper (smaller-support) gradients.
    """
    def __init__(
        self,
        input_size: int,
        ...
        super().__init__()
        if temperature <= .0:
            raise ValueError(f"Temperature should be a positive number: {temperature}")
        self.input_size: int = input_size
        self.temperature: float = float(temperature)
        # Shape of (1,) keeps the radius in the no-weight-decay group of the optimizer.
        self.radius: nn.Parameter = nn.Parameter(torch.tensor([float(initial_radius)]))
        distances: torch.Tensor = linalg.vector_norm(
            generate_centered_2d_coordinates_grid(input_size), dim=-1
        )
        self.register_buffer("distances", distances, persistent=False)

    def forward(self) -> torch.Tensor:
        """Computes the mask for the current value of the radius parameter.

        :return: A soft mask in [0, 1] of dimensionality: input_size x input_size.
        """
        return torch.sigmoid((self.radius - self.distances) / self.temperature)

    def get_radius(self) -> float:
        return self.radius.detach().item()
